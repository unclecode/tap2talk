...
// Platform Detection and Download Functions
...
function detectPlatform() {
    ...
    // Check for Mac first (includes Intel and Apple Silicon)
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const platformLinksText = document.getElementById('platformLinksText');
    const altPlatform1 = document.getElementById('altPlatform1');
    const altPlatform2 = document.getElementById('altPlatform2');

    if (detectedPlatform === 'mac') {
        downloadBtnText.textContent = 'Download for macOS - Talk in 2 Minutes';
        platformLinksText.textContent = 'Also available for ';
        altPlatform1.textContent = 'Windows';
        altPlatform1.onclick = function () { downloadWindows(); return false; };
        altPlatform2.textContent = 'Linux';
        altPlatform2.onclick = function () { downloadLinux(); return false; };
    } else if (detectedPlatform === 'windows') {
        downloadBtnText.textContent = 'Download for Windows - Talk in 2 Minutes';
        platformLinksText.textContent = 'Also available for ';
        altPlatform1.textContent = 'macOS';
        altPlatform1.onclick = function () { downloadMac(); return false; };
        altPlatform2.textContent = 'Linux';
        altPlatform2.onclick = function () { downloadLinux(); return false; };
    } else if (detectedPlatform === 'linux') {
        downloadBtnText.textContent = 'Download for Linux - Talk in 2 Minutes';
        platformLinksText.textContent = 'Also available for ';
        altPlatform1.textContent = 'macOS';
        altPlatform1.onclick = function () { downloadMac(); return false; };
        altPlatform2.textContent = 'Windows';
        altPlatform2.onclick = function () { downloadWindows(); return false; };
    }
});
...
async function getLatestRelease() {
    try {
        const response = await fetch('https://api.github.com/repos/unclecode/tap2talk/releases/latest');
        const data = await response.json();
        return data;
    } catch (error) {
        console.error('Failed to fetch release:', error);
        // Fallback to releases page
        window.location.href = 'https://github.com/unclecode/tap2talk/releases/latest';
        return null;
    }
}
...
async function downloadForPlatform() {
    if (detectedPlatform === 'mac') {
        ...
        // If platform unknown, go to releases page
        window.location.href = 'https://github.com/unclecode/tap2talk/releases/latest';
    }
}

// Download functions for each platform
async function downloadMac() {
    const release = await getLatestRelease();
    if (release && release.assets) {
        // Look for DMG file (macOS)
...
async function downloadWindows() {
    const release = await getLatestRelease();
    if (release && release.assets) {
        // Look for EXE file (Windows installer)
...
async function downloadLinux() {
    const release = await getLatestRelease();
    if (release && release.assets) {
        // Look for AppImage file (Linux)
        const appImageAsset = release.assets.find(asset =>
            asset.name.endsWith('.AppImage')
        );

        if (appImageAsset) {
            window.location.href = appImageAsset.browser_download_url;
        } else {
            window.location.href = release.html_url;
        }
    }
...
window.downloadMac = downloadMac;
window.downloadWindows = downloadWindows;
